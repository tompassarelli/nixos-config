...
** Level 3: Custom Module System with Enable Flags (Advanced) � *You Are Here*
...
** Level 4: Flakes + Multi-Host (Expert)
  ...
  outputs = { self, nixpkgs, home-manager, ... }: {
    nixosConfigurations = {
      ...
      desktop = self.lib.mkSystem {
        hostname = "tom-desktop";
        hostConfig = ./hosts/tom-desktop/configuration.nix;
      };
...
Most example configs are Level 0-1. Yours is Level 3-4. That's intentional complexity for:
- Multi-host management
- Modularity
- Shareability
- Learning the module system

** "Can I mix levels?"

Yes! Common pattern:
- Level 3-4 for your custom stuff (~myConfig.*~)
...
You're at Level 3-4. To go deeper:

1. *Learn the module system* (you're doing this now)
2. *Create shared profiles* (workstation.nix, server.nix)
3. *Add assertions* (validate your config at build time)
4. *Use lib functions* (reduce duplication)
5. *Contribute modules* (share your patterns)
